...
1. Author the formula record in `data/formulas/` with citations and a worked
   example (see `docs/architecture/CORPUS_DESIGN.md` for fields).
...
3. For a public page, add a calculator record in `data/calculators/` following
   `docs/planning/MVP_PAGE_SPECS.md`; its inputs must cover the formula's inputs (an input
...
1. `docs/architecture/CORPUS_DESIGN.md` with any new corpus fields.
2. `docs/architecture/TECHNICAL_ARCHITECTURE.md` if the data flow changes.
3. `docs/planning/DEVELOPMENT_PLAN.md` if scope or priority changes.
4. `docs/planning/MVP_PAGE_SPECS.md` if page standards or the MVP set change.
